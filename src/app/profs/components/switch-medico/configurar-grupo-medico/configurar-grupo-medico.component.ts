...
@Component({
  selector: 'app-configurar-grupo-medico',
  templateUrl: './configurar-grupo-medico.component.html',
  styleUrls: ['./configurar-grupo-medico.component.scss']
})
export class ConfigurarGrupoMedicoComponent implements OnInit {
  ...
  agregarEjercicio() {
    if (this.asignarEjercicio.valid) {
      const data = new FormData();
      data.append('receta', this.asignarEjercicio.value.ejercicio);

      const url = `${environment.apiUrl}/shared/rutinas/agregarReceta/`;
